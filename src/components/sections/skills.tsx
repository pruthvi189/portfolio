  ...
  return (
    <section id="skills" className="section-padding py-24">
      <div className="max-w-6xl mx-auto">
        <FadeIn>
          <p className="text-sand font-medium text-sm tracking-wide uppercase mb-3">
            Skills
          </p>
          ...
          {skillCategories.map((category) => (
            <StaggerItem key={category.name}>
              <div className="border border-border rounded-2xl p-6 bg-card">
                <h3 className="font-semibold text-lg mb-4 text-primary">{category.name}</h3>
                <div className="flex flex-wrap gap-2">
                  {category.skills.map((skill) => (
                    <span
                      key={skill}
                      className="text-sm text-muted-foreground bg-secondary border border-border px-3 py-1.5 rounded-lg"
                    >
                      {skill}
                    </span>
                  ))}
